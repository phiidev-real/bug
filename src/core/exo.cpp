...
namespace {
	...
	int ReadRoutingFile()
	{
		...
		EXO_MESSAGE("Routing table opened; " << (int)file_count << " file(s) to init. ");
		::PushTab();
...
//I should really reconsider this design... I don't like having to call other files to run this code,
//but it's the only way to keep swarm from having to interface directly with exo's stuff. 
//Creating a Model class would fix this
uint exo::AddToBuffers( uint32_t* model, seg::Segment* h_segment )
{
	...
	memcpy(h_segment->attribs_head, model + 2           , att_byte_count); h_segment->attribs_head += att_byte_count / sizeof(GLfloat);
	memcpy(h_segment->indices_head, model + 2 + model[0], ind_byte_count); h_segment->indices_head += ind_byte_count / sizeof(GLuint );
	
	h_segment->end += model[1];
